...
<template>
  <div class="settings-container">
    <aside>
      <ul class="tree-view">
        <li v-for="(screen, index) in screens" :key="index" :class="[{active: activeScreen == screen.content}]">
          <a v-if="screen.content" @click="activeScreen = screen.content">{{ screen.name }}</a>
          <span v-else>{{ screen.name }}</span>
          <ul v-if="screen.screens">
            <li v-for="(childScreen, childIndex) in screen.screens" :key="childIndex" :class="[{active: activeScreen == childScreen.content}]">
              <a v-if="childScreen.content" @click="activeScreen = childScreen.content" :focus="activeScreen == childScreen.content">{{ childScreen.name }}</a>
